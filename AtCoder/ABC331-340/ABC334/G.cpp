...
int main() {
    ...
    dynamic_connectivity<ll> uf(H*W+1);
    mint m = 0;
    rep(i, H){
        rep(j, W){
            if(G[i][j] == '.') m += 1;
                ...
                rep(k, 4){
                    ...
                    if(G[nx][ny] == '.') uf.link(toid(i, j), H*W);
                    else uf.link(toid(i, j), toid(nx, ny));
                }
            }
        }
    }
    ...
    rep(i, H){
        rep(j, W){
            ...
            rep(k, 4){
                ...
                if(nx < 0 || nx >= H || ny < 0 || ny >= W) continue;
                if(G[nx][ny] == '.') continue;
                uf.cut(toid(i, j), toid(nx, ny));
                // st.insert(uf.root(toid(nx, ny)));
                uf.link(toid(i, j), toid(nx, ny));
            }
            ll d = cnct_cnt - 1 + st.size();
            ans += d;
        }
    }
    ans /= m;
