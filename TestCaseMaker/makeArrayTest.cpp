...
vector<ll> make_array_test(ll siz, bool is_unique = false, ll left = 1, ll right = 100) {
    ...
    uniform_int_distribution<ll> dist_N(left, right);
    vector<ll> res;
    if(is_unique) {
        set<ll> st;
        while(st.size() < siz) {
...
vector<vector<ll>> make_test_case(ll q, ll minsiz = 1, ll maxsiz = 100, bool is_unique = false, ll left = 1, ll right = 100) {
    ...
    mt19937_64 rnd(chrono::duration_cast<chrono::nanoseconds>(chrono::system_clock::now().time_since_epoch()).count());
    uniform_int_distribution<ll> dist_N(minsiz, maxsiz);
    rep(i, q) {
        ll siz = dist_N(rnd);
        res.push_back({siz});
...
int main(int argc, char* argv[]) {
    //Configurations
    ll q = 100;  // number of testcases
    ll minsiz = 1; // minsize of array
    ll maxsiz = 1e5;    // maxsize of array
    bool is_unique = true; // whether the array is unique or not
    ll left = 1;    // minimum value of array
    ll right = 1e9; // maximum value of array
    string foldername = "ArrayTest"; // folder to write the test
    string filename = "large-unique"; // filename to write the test

    auto test = make_test_case(q, minsiz, maxsiz, is_unique, left, right);
    write_file(foldername + "/" + filename + ".txt", test);
    return 0;
}
