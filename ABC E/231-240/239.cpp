...
Graph G;
vector<ll> ord, lb, rb, id;//ord:オイラーツアー順序, id[x]:xを最初に訪れたのがオイラーツアー順序の何番目か
...
int main() {
    ...
    id.resize(N);lb.resize(N);rb.resize(N);
    vector<ll> V(Q), K(Q);
    rep(i, Q) cin>>V.at(i)>>K.at(i);
    vector<set<ll>> dp(N);
    dfs(0, -1);
    WaveletMatrix<ll> WM(ord.size());
    rep(i, ord.size()){
        WM.set(i, 0);
    }
    rep(i, id.size()){
        WM.set(id.at(i), X.at(i));
    }
    WM.build();
    rep(i, Q){
        ll v = V.at(i);
        ...
        ll k = K.at(i);
        k--;
        out(WM.kth_largest(lb[v], rb[v]+1, k));
    }
}