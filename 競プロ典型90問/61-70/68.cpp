...
int main() {
    ...
    rep(i, Q) cin>>T.at(i)>>X.at(i)>>Y.at(i)>>V.at(i);
    vector<ll> edge(N-1, 0);
    rep(i, Q){
        X.at(i)--;Y.at(i)--;
        if(T.at(i) == 0){
            edge.at(X.at(i)) = V.at(i);
        }
    }
    //A_0を0とするポテンシャル
    vector<ll> potential(N, 0);
    rep(i, N-1){
        potential.at(i+1) = edge.at(i) - potential.at(i);
    }
    UnionFind uf(N);
    rep(i, Q){
        if(T.at(i) == 0){
            ...
            if(!uf.issame(X.at(i), Y.at(i))){
                out("Ambiguous");
            }
            else if((X.at(i) + Y.at(i)) % 2 == 0){
                out(V.at(i) + (potential.at(Y.at(i)) - potential.at(X.at(i))));
            }
            else{
                out((potential.at(X.at(i)) + potential.at(Y.at(i)) - V.at(i)));
            }
        }
    }
}